...
export async function POST(request: NextRequest) {
  ...
  // Idempotency: Stripe delivers events at-least-once, so skip one we've already
  ...
  try {
    ...
    if (dupErr) console.warn("stripe idempotency insert skipped:", dupErr.message);
  } catch (e) {
    console.warn("stripe idempotency check errored, processing anyway", e);
  }
